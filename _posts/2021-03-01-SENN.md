...
{% capture caption %}
<blockquote>
Images sampled form the latent space of the style encoder manipulating a single dimension of the latent space.
</blockquote>
{% endcapture %} 
{% include figure.html src=newpath caption=caption width="400px" %}

As already hinted at above, in practice encoder <span>$q\_\phi(z|x)$</span>  and decoder <span>$p\_\theta(x|z)$</span> are often modeled by two neural networks composed as in the network architecture depicted in figure \ref{figure:vae}.
...
PCA             |  t-SNE
:-:|:-:
![]({{page.img_dir}}pca_mnist_senn_1e1.png )*Senn*  |  ![]({{page.img_dir}}tsne_mnist_senn_1e1.png )
![]({{page.img_dir}}pca_mnist_vae_1e1.png )  |  ![]({{page.img_dir}}tsne_mnist_vae_1e1.png )
