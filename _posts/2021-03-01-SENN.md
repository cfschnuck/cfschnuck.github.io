...
A *SENN* consists of two components: a conceptizer <span>$h(x)$</span> and a parametrizer <span>$\theta(x)$</span>. The conceptizer aims to encode input data as meaningful concepts while the parametrizer learns explanations which are then applied to concepts to obtain predictions. In its simplest version a *SENN* then aggregates conceptizer and parametrizer using an inner product such that we can write <span>$f(x) = \langle h(x), theta(x)\rangle$</span>.
...
<span>$||\nabla\_x f(x) - \theta(x)^\intercal J\_x^h(x)||$</span> 

which is high (and consequently penalizes more) if the derivative of the whole SENN wrt. input x differs greatly from the derivative of a SENN with fixed parametrizer.
